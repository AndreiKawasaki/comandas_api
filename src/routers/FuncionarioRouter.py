...
async def post_funcionario(
    ...
    try:
        ...
        hashed = get_password_hash(funcionario_data.senha)
        novo = FuncionarioDB(
            id=None,
...
async def put_funcionario(
    ...
    try:
        ...
        if funcionario_data.senha:
            funcionario_data = funcionario_data.model_copy(
                update={"senha": get_password_hash(funcionario_data.senha)}
            )
...
async def delete_funcionario(
    ...
    """Apenas grupo 1; não permite excluir a si mesmo."""
    try:
        funcionario = db.query(FuncionarioDB).filter(FuncionarioDB.id == id).first()
        if not funcionario:
            raise HTTPException(
